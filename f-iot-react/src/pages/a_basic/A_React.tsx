...
# JavaCript VS React
? 화면 변경 방식
JS: DOM 직접 조작 (document.querySelector, innerHTML 등)
React: 상태(state)가 바뀌면 자동으로 필요한 부분만 업데이트
# Virtual DOM 으로 필요한 부분만 최소 변경
...
- 라우팅 (Routing): 페이지 간의 전환을 'React Router' 
라이브러리를 통해서 구현 (페이지 간 전환 관리)

# a태그 VS Link 비교
&    a (HTML)                link (React Router)
- 서버에 새 html 요청       - 브라우저 내에서 URL만 변경
- 새로고침 발생             - 새로고침 X
- 느림(전체 새로 load)      - 빠름 (필요한 부분만 갱신)
- 정적 사이트               - 리액트 
